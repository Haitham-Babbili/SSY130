...
size_t n_lms_taps = 340;	//Number of LMS taps to use right now
...
void my_lms(float const * y, float const * x, float * xhat, float * e, int block_size,
	...
	*/
	 ...
	 * 	xhat[n]+=lms_coeffs[k]*lms_state[n+k];
	 * }
	 * e[n] = x[n] - xhat[n]; 
	 * for(i=0; i<lms_taps-1; i++){
	 * 	lms_coeffs[11]+=2*lms_mu*e[n]*lms_state[n+i];
	 * 	}
	 * }
	 *  * ...to here */
	  /*int n ;
	  ...
	  for ( n=0; n<block_size ; n++){
		...
		e[n] = (x[n] - xhat[n]) ;
		arm_scale_f32 ( lms_state+n , e[n]*(2*lms_mu) , y_u , lms_taps ) ;
		arm_add_f32 ( y_u , lms_coeffs , lms_coeffs , lms_taps ) ;
	 }*/
