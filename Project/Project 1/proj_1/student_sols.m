...
% directory as this file as well as any of the standard matlab functions.
	...
	function z = add_cyclic_prefix(x,N_cp )  %#ok<*INUSD>
		% Adds (prepends) a Ncp long cyclic prefix to the ofdm block x.
        %x=[1 2 5 6 7 8];
        %x=x'
		x = x(:);   %#ok<*NASGU> % Ensure x is a column vector % 128x1
		z = [x(end - N_cp  +1:end);x]; %TODO: This line is missing some code!
    end

    function x = remove_cyclic_prefix(z,N_cp )
        % Removes a Ncp long cyclic prefix from the ofdm package z
        z = z(:);   % Ensure z is a column vector
        x = z(N_cp +1:end); %TODO: This line is missing some code!
    end
    ...
    function symb = bits2qpsk(bits)
        ...
        realparts = bits(1:2:end);
        imgpart= bits(2:2:end);
        symb =  (realparts +1j* imgpart)/sqrt(2); %TODO: This line is missing some code!
    end
    ...
    function [rx, evm, ber, symbs] = sim_ofdm_known_channel(tx, h, N_cp, snr, sync_err)
        ...
        rx = qpsk2bits(r); %TODO: This line is missing some code!
    ...
    function [rxPilot, rxData] = split_frame(rxFrame)
        ...
        N = length(rxFrame);
        p=N/2;
        rxPilot = rxFrame(1:p); %TODO: This line is missing some code!
        rxData = rxFrame(p+1:end); %TODO: This line is missing some code!
    end
    ...
    function [rx, evm, ber, symbs] = sim_ofdm_audio_channel(tx, N_cp, snr, sync_err, f_s, f_c, L)
        ...
        % Convert to frequency domain using DFT
        r.p = remove_cyclic_prefix(ycp.p,N_cp); %TODO: This line is missing some code!
        r.d = remove_cyclic_prefix(ycp.d,N_cp); %TODO: This line is missing some code!
        symbs.rx_pe = r.d; % Store symbols for later
